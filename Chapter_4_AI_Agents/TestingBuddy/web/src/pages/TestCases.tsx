...
const TestCases = () => {
  ...
  const handleGenerate = async () => {
    ...
    try {
      // Mock calling the LLM directly for test cases
      const response = await fetch('http://localhost:3001/api/generate/test-plan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ticketDetails, options: { includeTestCases: true, functional: true, security: true }, llmConnection: activeLLM })
      });
      const data = await response.json();
  ...
  const handleDownload = async () => {
    if (!generationResult?.cases) return;
    try {
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <h1 className="heading-1">Generate Test Cases</h1>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '0.5rem', marginBottom: '2rem', display: 'flex', gap: '1rem', alignItems: 'flex-end' }}>
        ...
        <div style={{ flex: 2 }}>
          <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500 }}>Enter Requirement ID</label>
          <input type="text" className="input-field" value={ticketId} onChange={(e) => setTicketId(e.target.value)} placeholder="e.g. REQ-999" />
        </div>
      ...
      {ticketDetails && (
        <div className="glass-panel" style={{ padding: '2rem', borderRadius: '0.5rem' }}>
          ...
          <h2 className="heading-2">Generate Test Case</h2>
          ...
          <button className="btn btn-primary" onClick={handleGenerate} disabled={isGenerating}>
            {isGenerating ? <Loader2 className="animate-spin" size={18} /> : 'Generate Cases'}
          </button>
          ...
          {generationResult && (
            <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid var(--border-color)' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                <button className="btn btn-outline" onClick={() => { setPreviewData(generationResult.cases || generationResult); setIsPreviewOpen(true); }}><Eye size={18} /> Preview Cases</button>
                <button className="btn btn-outline" onClick={handleDownload}><Download size={18} /> Download Test Cases</button>
              </div>
            </div>
          )}
        </div>
