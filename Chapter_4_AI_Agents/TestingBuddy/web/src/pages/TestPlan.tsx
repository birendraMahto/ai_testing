...
const TestPlan = () => {
  ...
  const handleGenerate = async () => {
    if (!ticketDetails || !activeLLM) return;
  ...
  const handleDownloadPlan = async () => {
    if (!generationResult?.plan) return;
    try {
      ...
      a.href = url;
      a.download = `TestPlan_${ticketId}.docx`;
  ...
  const handleDownloadCases = async () => {
    if (!generationResult?.cases || generationResult.cases.length === 0) return;
    try {
      const response = await fetch('http://localhost:3001/api/download/cases', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ cases: generationResult.cases })
      });
      const blob = await response.blob();
      const url = window.URL.createObjectURL(blob);
      const a = document.createElement('a');
      a.href = url;
      a.download = `TestCases_${ticketId}.xlsx`;
      document.body.appendChild(a);
      a.click();
      a.remove();
    } catch (err) {
  ...
  const canDownloadTestCases = generationResult?.cases?.length > 0;

  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <h1 className="heading-1">Create Test Plan</h1>
      
      {error && (
        <div style={{ padding: '1rem', backgroundColor: 'var(--danger)', color: 'white', borderRadius: '0.375rem', marginBottom: '1rem' }}>
          {error}
        </div>
      )}
      ...
      <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '0.5rem', marginBottom: '2rem', display: 'flex', gap: '1rem', alignItems: 'flex-end' }}>
        ...
        <div style={{ flex: 2 }}>
          <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500 }}>Enter Ticket ID (Requirement/Epic/Story)</label>
      ...
      {ticketDetails && (
        <div className="glass-panel" style={{ padding: '2rem', borderRadius: '0.5rem', marginBottom: '2rem' }}>
          <h2 className="heading-2">Ticket Details</h2>
          ...
          <h2 className="heading-2">Generate Test Plan</h2>
          ...
          {generationResult && (
            <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid var(--border-color)' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                <button className="btn btn-outline" onClick={() => { setPreviewData(generationResult.plan); setIsPreviewOpen(true); }}><Eye size={18} /> Preview Plan</button>
                <button className="btn btn-outline" onClick={handleDownloadPlan}><Download size={18} /> Download Test Plan</button>
                <button className="btn btn-outline" onClick={() => { setPreviewData(generationResult.cases); setIsPreviewOpen(true); }} disabled={!canDownloadTestCases}><Eye size={18} /> Preview Cases</button>
                <button className="btn btn-outline" onClick={handleDownloadCases} disabled={!canDownloadTestCases}><Download size={18} /> Download Test Case</button>
              </div>
            </div>
          )}
        </div>
