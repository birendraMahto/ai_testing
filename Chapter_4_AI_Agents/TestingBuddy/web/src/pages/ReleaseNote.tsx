...
const ReleaseNote = () => {
  const { toolConnections } = useAppContext();
  ...
  const handleGenerate = async () => {
    ...
    try {
      const response = await fetch('http://localhost:3001/api/generate/test-plan', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ ticketDetails, options: { includeTestCases: false }, llmConnection: null })
      });
      const data = await response.json();
      if (!response.ok) {
  ...
  const handleDownload = async () => {
    ...
    try {
      ...
      a.href = url;
      a.download = `ReleaseNotes_${ticketId}.docx`;
  ...
  return (
    <div style={{ maxWidth: '900px', margin: '0 auto' }}>
      <h1 className="heading-1">Generate Release Note</h1>
      ...
      <div className="glass-panel" style={{ padding: '1.5rem', borderRadius: '0.5rem', marginBottom: '2rem', display: 'flex', gap: '1rem', alignItems: 'flex-end' }}>
        ...
        <div style={{ flex: 2 }}>
          <label style={{ display: 'block', marginBottom: '0.5rem', fontWeight: 500 }}>Enter Epic/Release ID</label>
          <input type="text" className="input-field" value={ticketId} onChange={(e) => setTicketId(e.target.value)} placeholder="e.g. RELEASE-v1.2" />
        </div>
      ...
      {ticketDetails && (
        <div className="glass-panel" style={{ padding: '2rem', borderRadius: '0.5rem' }}>
          ...
          <h2 className="heading-2">Generate</h2>
          <button className="btn btn-primary" onClick={handleGenerate} disabled={isGenerating}>
            {isGenerating ? <Loader2 className="animate-spin" size={18} /> : 'Generate Release Notes'}
          </button>
          ...
          {generationResult && (
            <div style={{ marginTop: '2rem', paddingTop: '2rem', borderTop: '1px solid var(--border-color)' }}>
              ...
              <div style={{ display: 'flex', gap: '1rem' }}>
                <button className="btn btn-outline" onClick={() => { setPreviewData(generationResult.plan); setIsPreviewOpen(true); }}><Eye size={18} /> Preview Notes</button>
                <button className="btn btn-outline" onClick={handleDownload}><Download size={18} /> Download Notes</button>
              </div>
            </div>
          )}
        </div>
